...
public class regex extends ModuleBody {
    ...
    public static CharSequence regexReplace$St(Object re, CharSequence str, Object repl) {
        ...
        rex = Pattern.compile(re.toString());
        frame.matcher = rex.matcher(str);
        frame.sbuf = new StringBuffer();
        if (misc.isProcedure(frame.repl)) {
            frame.loop = frame.loop;
            return frame.lambda1loop();
        }
        Matcher matcher = frame.matcher;
        Object obj = frame.repl;
