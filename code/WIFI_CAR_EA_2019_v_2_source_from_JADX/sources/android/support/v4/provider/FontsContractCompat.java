...
public class FontsContractCompat {
    ...
    private static final Comparator<byte[]> sByteArrayComparator = new C01085();
    ...
    static class C01085 implements Comparator<byte[]> {
        C01085() {
        }
    ...
    public static void requestFont(@NonNull final Context context, @NonNull final FontRequest request, @NonNull final FontRequestCallback callback, @NonNull Handler handler) {
        ...
        handler.post(new Runnable() {
            ...
            class C01036 implements Runnable {
                C01036() {
                }
            ...
            class C01058 implements Runnable {
                C01058() {
                }
                ...
                try {
                    ...
                    if (result.getStatusCode() != 0) {
                        switch (result.getStatusCode()) {
                            case 1:
                                callerThreadHandler.post(new C00992());
                                return;
                            case 2:
                                callerThreadHandler.post(new C01003());
                                return;
                            default:
                                callerThreadHandler.post(new C01014());
                                return;
                        }
                    }
                    FontInfo[] fonts = result.getFonts();
                    if (fonts == null || fonts.length == 0) {
                        callerThreadHandler.post(new C01025());
                        return;
                    }
                    for (FontInfo font : fonts) {
                        if (font.getResultCode() != 0) {
                            ...
                            if (resultCode < 0) {
                                callerThreadHandler.post(new C01036());
                                return;
                            } else {
                    ...
                    if (typeface == null) {
                        callerThreadHandler.post(new C01058());
                    } else {
                        callerThreadHandler.post(new Runnable() {
                            public void run() {
                    ...
                    callerThreadHandler.post(new C00981());
                }
            }
        });
    }
